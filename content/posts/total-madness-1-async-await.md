...
I introduced a field called `run_counter` that starts with zero and is incremented every time we call `run()`. When the counter is `100` and we call `run()`, the `done` field is set to `true` and the task returns. Then our executor will clean it up since it's `done`. What this effectively does is make every task print "Hi from task {name}" 100 times. For our purposes, this simulates the task being run partially every time we call `run()` on it.

But ideally we want our tasks to be general, that is, we want to create a task and pass a function that it'll execute, rather than having it hardcoded in the `run()` method. To do that, let's go back to our tooth brushing example from before:

TODO: aunt Fefê brushes 100 times, I brush 20, Nat brushes 30; futures, context, await
